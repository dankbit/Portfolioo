...
import { close3, logojpg,menu2 } from '../assets';
...
const Navbar = () => {
  ...
  return (
    <nav
      className=' p-4 w-full flex items-center py-0 fixed 
      top-0 z-20 bg-zinc-900 custom:opacity-[0.97] xxs:h-[12vh] mt-[-8px]'>
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto my-0 mt-[-8px]">
        <Link
          to="/"
          ...
          <img
            ...
            className="custom:w-[50px] custom:h-[50px] w-[45px] h-[45px] object-contain"
          />

         
          <span className='font-bold text-4xl text-white font-sans'> Utsav</span>
         
        </Link>
        <ul className="list-none hidden custom:flex flex-row gap-14 mt-2">
          {navLinks.map((nav) => (
            <li
              key={nav.id}
              className={`${
        ...
        {/* mobile */}
        {/* <div className="custom:hidden flex flex-1 w-screen justify-end items-center">
          {toggle ? (
            <div
              ...
              <ul
                ...
                items-start justify-end mt-[3rem] -ml-[20px]">
                {navLinks.map((nav) => (
                  <li
                    id={nav.id}
                    key={nav.id}
